        ...
        for batch_idx, data in enumerate(loader):
            for batch_idx1, data1 in enumerate(loader0):
                if batch_idx1 == batch_idx:
                    ...
                    total_labels_p = torch.cat(
                        (total_labels_p, data["y"].float().view(-1, 1).cpu()), 0
                    )
...
if __name__ == "__main__":
    ...
    seed = 1500
    tranditional_train = data_syn.tranditional_model_data(
        train_rate=0.9, val_rate=0.1, data_path=data_path, seed=seed
    )
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for iteratation in range(0, train_len):
            train_data0 = train_sample[
                iteratation * TRAIN_BATCH_SIZE : (iteratation + 1) * TRAIN_BATCH_SIZE
            ]  # slice a batch from the shuffled training samples
            train_data = TestbedDataset(
                train_data0, drug_features, cell_features, codes
            )
            ...
            train_loader1 = DataLoader(
                train_data1, batch_size=TRAIN_BATCH_SIZE, shuffle=False
            )
            loss_all = train(
                EMB, device, train_loader, train_loader1, optimizer, scheduler
            )
            aver_loss += loss_all
            log_interval = 30
            if iteratation % log_interval == 0 and iteratation > 0:
                ...
                print(
                    "| epoch {:3d} | {:5d}/{:5d} batches |  loss {:8.5f}".format(
                        epoch,
                        iteratation,
                        int(len(train_loader) / TRAIN_BATCH_SIZE),
                        cur_loss,
                    )
                )
                aver_loss = 0
        ...
        if epoch % 10 == 0:
            ...
            for iteratation_val in range(0, val_len):
                val_sample = df_val[
                    iteratation_val
                    * VAL_BATCH_SIZE : (iteratation_val + 1)
                    * VAL_BATCH_SIZE
                ]
                val_data = TestbedDataset(
                    val_sample, drug_features, cell_features, codes
                )
                ...
                val_loader1 = DataLoader(
                    val_data1, batch_size=VAL_BATCH_SIZE, shuffle=False
                )
                total_labels, total_preds = predicting(
                    EMB, device, val_loader, val_loader1, total_labels, total_preds
                )
