...
class MiniCellDataSet:
    ...
    def sample_new_batch(self, data_pack):
        ...
        support_set_x = np.zeros(
            (self.batch_size, self.samples_support, 4), dtype=np.float64
        )
        ...
        for i in range(self.batch_size):
            ...
            for x in choose_cells:
                ...
                choose_samples = np.random.choice(
                    samples_idx,
                    size=self.samples_support + self.samples_query,
                    replace=False,
                )
            ...
            target_x[i] = x_temp[
                self.samples_support : (self.samples_support + self.samples_query)
            ]
...
class tranditional_model_data:
    def __init__(
        ...
        np.random.seed(seed)
        self.x_train, self.x_val, self.x_test, self.x_all_data = load_Synergydata(
            data_path
        )
        if shuffle_cells:
            cell_ids = np.arange(len(self.x_train))
    ...
    def get_batch(self, dataset_name, augment=False):
        ...
        :return:
        """
        train_train, train_val, train_test = self.sample_new_batch(
