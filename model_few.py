...
class Feature_embedding_network(torch.nn.Module):
    ...
    def forward(self, data_train, data0_train, task_batch):
        # Drug A
        x1_train, edge_index1_train, batch1_train = (
            data_train["x"],
        ...
        # Drug B
        x2_train, edge_index2_train, batch2_train = (
            data0_train["x"],
            data0_train["edge_index"],
            data0_train["batch"],
        )
...
class HyperSynergy(nn.Module):
    def __init__(self, num_support, num_query):
        ...
        self.inner_l_rate = nn.Parameter(
            torch.FloatTensor([0.1]), requires_grad=True
        )  # A learnable parameter, indicating an adaptive approach to updating the model's weights during training.
    ...
    def run_batch(self, data_train, data0_train, batch_size, train1=True):
        ...
        latents, kl_div, encoder_penalty = self.meta_train_batch(
            support_embedings, support_labels
        )
        ...
        # calculate loss
        total_loss = (
            val_loss + kl_weight * kl_div + encoder_penalty_weight * encoder_penalty
        )
    ...
    def meta_train_batch(self, inputs, target):
        ...
        for i in range(15):  # updata latents
            ...
            train_loss, _ = self.cal_target_loss(
                inputs, specific_weights, target, drop=0.0
            )
        ...
        kl_div = F.kl_div(
            latents.softmax(dim=-1).log(),
            z_nomal_singal.softmax(dim=-1),
            reduction="mean",
        )
    ...
    def inner_finetuning(self, latents, inputs, target, val_input, val_target, train):
        ...
        val_loss, val_output = self.cal_target_loss(
            val_input, specifci_weights, val_target, drop=0
        )
        if train == False:
